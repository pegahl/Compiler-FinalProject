...
import compiler.symbols.MethodSymbol;
import compiler.symbols.ParameterSymbol;
import compiler.symbols.StatementSymbol;
...
public class ErrorHandler implements CListener {
...
//            }
//            FieldSymbol symbol = new FieldSymbol(name, type, row, column);
...
//            }
            ParameterSymbol symbol = new ParameterSymbol(name, type, row, column);
            currentScope.insert(symbolKey, symbol);
...
//        int row = ctx.getStart().getLine();
//        int column = ctx.getStart().getCharPositionInLine();
//        String forName = "for_" + row + "_" + column;
//        currentScope = scopeMap.getScope(forName);
...
//        System.out.println("methodname: "+methodName+" type: "+returnType);

        int row = ctx.getStart().getLine();
        System.out.println("scope is"+s);
        int column = ctx.getStart().getCharPositionInLine();
...
//
//            String parameterName =ctx.parameterDeclaration().get(i).declarator().start.getText();
//            String parameterType = ctx.parameterDeclaration().get(i).declarationSpecifiers().getText();
...
//        int row = ctx.getStart().getLine();
        ...
        if (scope.getChildren().size() == 0) {return;}
        for (Scope s : scope.getChildren()) {printScopes(s);}
    }
    ...
    private boolean lookUpScopes(Scope scope, String key, String type) {
        ...
        if ("method".equals(type)) {
            ...
            if (scope.LookUp(key + "_field")) {
                ...
                if (scope.LookUp(key + "_parameter")) {
                    return true;
                }
    ...
    public void printRedefineError(String name, String type, int row, int column) {
        ...
        String errorString =  String.format("%s : in line[%d:%d], %s [%s] has been defined already",errorNumber, row, column, type, name);
    ...
    public void printUndefinedError(String name, String type, int row, int column) {
        ...
        String errorString =  String.format("%s : in line[%d:%d],can't find %s [%s]",errorNumber, row, column, type, name);
        System.out.println(errorString);
    }
